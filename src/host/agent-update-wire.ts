...
export function parseAgentUpdate(value: unknown): AgentUpdate | undefined {
    ...
    if (isOneshotReplyType(update.type)) {
        return update.seq === undefined
            ? parseOneshotReply(value, update)
            : undefined;
    }
...
function isOneshotReplyType(value: unknown): boolean {
    return value === "oneshot_result" || value === "oneshot_rejected";
}

function parseOneshotReply(
    value: unknown,
    update: Record<string, unknown>,
): AgentUpdate | undefined {
