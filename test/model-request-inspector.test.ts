...
test("a session logged before sharding is still inspectable", async () => {
    const root = await mkdtemp(join(tmpdir(), "vera-request-inspection-"));
    const sessionPath = join(root, "session.jsonl");
    ...
    try {
        ...
        // The flat location, which is where every log written before the
        // layout changed still sits.
        await writeFile(
            join(logRoot, "session-1.jsonl"),
            `${JSON.stringify(requestEvent("session-1", "old-model", "old"))}\n`,
        );

        const inspected = JSON.parse(
            await inspectLatestModelRequest(sessionPath, undefined, logRoot),
        );
        expect(inspected.request.model).toBe("old-model");
    } finally {
        await rm(root, { recursive: true, force: true });
    }
});

test("the sharded location wins when both exist", async () => {
    const root = await mkdtemp(join(tmpdir(), "vera-request-inspection-"));
    const sessionPath = join(root, "session.jsonl");
    const logRoot = join(root, "logs");
    try {
        ...
        await mkdir(join(logRoot, workspaceKey("/work/vera")), {
            recursive: true,
        });
        await writeFile(
            join(logRoot, "session-1.jsonl"),
        ...
        await writeFile(
            join(logRoot, workspaceKey("/work/vera"), "session-1.jsonl"),
            `${JSON.stringify(requestEvent("session-1", "new-model", "new"))}\n`,
        );

        const inspected = JSON.parse(
            await inspectLatestModelRequest(sessionPath, undefined, logRoot),
        );
        expect(inspected.request.model).toBe("new-model");
    } finally {
        await rm(root, { recursive: true, force: true });
    }
});
