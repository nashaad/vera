...
test("Verify this model checks the row the list is on", () => {
    const menu = handleTuiSettingsPickerKey(base, more).state!;
    const at = menu.options.findIndex((row) => row.value === "verify_selected");
    expect(at).toBeGreaterThan(-1);
    const transition = handleTuiSettingsPickerKey({ ...menu, selectedIndex: at }, { name: "enter" });
    expect(transition.poolVerify).toEqual({ provider: "p", model: "a" });
});
