...
test("Manage shows Working until save completes, then success or failure below the list", async () => {
    ...
    const session = await startTuiTestSession({ home: mkdtempSync(join(tmpdir(), "vera-shortlist-feedback-")), width: 120, height: 36,
        dependencies: () => ({ ...createTuiCatalogRefreshDependencies({ pooled: [] }),
            operateModels: async (operation, onResult) => {
                calls++;
                if (calls > 1) throw new Error("Could not save shortlist: disk full");
                await gate;
    ...
    try {
        ...
        finish();
        const success = await session.waitForVisiblePane("✓ One added to shortlist");
        expect(success).toContain("kept ✓");
        expect(success).not.toContain("default slot");
        session.sendKey("Enter");
        const failed = await session.waitForVisiblePane("✗ Could not save shortlist: disk full");
        expect(failed).toContain("kept ✓");
        expect(failed).not.toContain("✓ One added");
        expect(failed).not.toContain("removed from shortlist");
