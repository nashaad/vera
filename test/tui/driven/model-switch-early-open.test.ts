...
import { createLockedJourney } from "../../locked/support/locked-journey.ts";
...
async function openSwitchModel(session: TuiTestSession): Promise<void> {
    session.sendKey("C-x");
    session.sendText("m");
    // No wait for the list: typing before Home has the host's settings is the case under test.
}

test("a switch typed before Home has the model list still switches", async () => {
