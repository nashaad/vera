...
test("long tool output folds and Ctrl-E reveals it", async () => {
    ...
    try {
        ...
        // The verb and the duration both move with machine load; the row's
        // shape is the assertion, matching the tmux test's fast-machine
        // "Baked for 0.0s" without inheriting its timing luck.
        expect(pane).toMatch(
            /^ {2}(?:Baked|Brewed|Churned|Cogitated|Cooked|Crunched|Sautéed|Worked) for \d+\.\d+s\n {2}Ran/m,
        );
