...
import type { SessionCheckpointEntry } from "../../src/store/session-store.ts";
...
test("checkpoint commands reject explicitly when no owner is installed", async () => {
    const channel = createInProcessChannel();
    const events = new EngineEventBus();
    events.subscribe(createProtocolEncoder(channel.engine));
    new InboundCommandRouter(channel.engine, events);
...
test("the router lists and restores checkpoints through its owner", async () => {
    const channel = createInProcessChannel();
    const events = new EngineEventBus();
    events.subscribe(createProtocolEncoder(channel.engine));
    ...
    channel.client.send({ type: "list_checkpoints", requestId: "list" });
    expect(await channel.client.receive()).toEqual({
        type: "checkpoints",
        requestId: "list",
        checkpoints: [entry],
...
test("restore rejects while a turn is active", async () => {
    const channel = createInProcessChannel();
    const events = new EngineEventBus();
    events.subscribe(createProtocolEncoder(channel.engine));
    let restoreCalls = 0;
    ...
    const pendingTurn = router.startTurn();
    channel.client.send({ type: "prompt", content: "keep working" });
    await pendingTurn;
    channel.client.send({
        type: "restore_checkpoint",
        requestId: "restore",
...
test("restore surfaces the owner's failure reason and never throws out", async () => {
    const channel = createInProcessChannel();
    const events = new EngineEventBus();
    events.subscribe(createProtocolEncoder(channel.engine));
    new InboundCommandRouter(channel.engine, events, {
