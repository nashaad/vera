...
test("verification steps reach the client before the result", async () => {
    const root = await mkdtemp("/tmp/vera-model-steps-");
    const socketPath = join(root, "host.sock");
    const server = await startHostServer({ socketPath, lockPath: join(root, "host.json"),
    ...
    try {
        const seen: string[] = [];
        await operateModelsThroughHost(socketPath, { operation: "verify", models: [{ provider: "p", model: "one" }] },
            (row) => seen.push(`result ${row.status}`), undefined, (step) => seen.push(`step ${step.label} ${step.status}`));
        expect(seen).toEqual(["step Model responds running", "step Model responds passed", "result passed"]);
    } finally { await server.close(); await rm(root, { recursive: true, force: true }); }
});
