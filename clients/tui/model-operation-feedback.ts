...
export function shortlistOperationFeedback(
    ...
    if (error !== undefined || failed !== undefined) return {
        status: "error", message: error ?? failed?.reason ?? "Could not update shortlist.",
    };
    ...
    if (!confirmed) return { status: "error", message: "Could not confirm the shortlist change." };
    const subject = operation.models.length === 1 ? label : `${operation.models.length} models`;
    return { status: "success", message: `${subject} ${operation.operation === "keep" ? "added to" : "removed from"} shortlist` };
}
