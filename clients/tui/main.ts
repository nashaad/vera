...
import { HOST_CAPABILITY_AGENT_ATTACHMENT_RELEASE, HOST_CAPABILITY_HARNESS_MESSAGES, HOST_CAPABILITY_SESSION_SCOPED_STATE, HOST_CAPABILITY_SKILL_COMMANDS } from "../../src/host/capabilities.ts";
import {
...
import { createConfiguredBuiltinTuiCommandRegistry, extensionCommandResultText, registerExtensionTuiCommands, registerSkillTuiCommands, renderTuiArgumentSuggestions, renderTuiCommandSuggestions, SLASH_COMPACT_WIDTH, tuiCommandSuggestionWidth, tuiSuggestionGaps, tuiSuggestionWindow, tuiArgumentSuggestions, tuiCommandArgumentHint, tuiCommandScope, type TuiCommandAction, type TuiCommandCatalogEntry, type TuiPaletteEntry } from "./commands.ts";
...
import { createTuiPreferencesListView, handleTuiPreferencesListScroll, startTuiPreferencesList, syncTuiPreferencesList } from "./preferences-list.ts";
...
import { applyTuiTimelineReply, createTuiTimelinePickerView, startTuiTimelinePicker } from "./timeline-picker.ts";
...
export function submitPrompt(rt: TuiRuntime, 
    ...
    if (commandAction?.type === "create_session") {
        beginCreateSession(rt, commandAction.sourceDisposition ?? "stop");
        return;
    }
